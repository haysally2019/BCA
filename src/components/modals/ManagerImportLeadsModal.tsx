...
export const ManagerImportLeadsModal: React.FC<ManagerImportLeadsModalProps> = ({
  ...
  const handleStartImport = async () => {
    ...
    const activeReps = teamMembers.filter(
      member => member.employment_status === 'active' && member.profile?.user_role === 'sales_rep'
    );

    if (activeReps.length === 0) {
      toast.error('No active sales reps found. Please add sales reps before importing leads.');
      return;
    }
    ...
    try {
      if (validLeads.length > 0) {
        const importResults = await bulkCreateLeadsWithDistribution(validLeads, activeReps);

        result.success = importResults.success;
        result.dbDuplicates = importResults.dbDuplicates;

        if (importResults.success > 0) {
          toast.success(`Successfully imported and distributed ${importResults.success} lead${importResults.success > 1 ? 's' : ''}`);
        }
      } else {
  ...
  const bulkCreateLeadsWithDistribution = async (
    leadsData: any[],
    activeReps: TeamMember[]
  ): Promise<{ success: number; failed: number; dbDuplicates: number }> => {
    const leadsPerRep = Math.ceil(leadsData.length / activeReps.length);
    let totalSuccess = 0;
    let totalDbDuplicates = 0;

    for (let i = 0; i < activeReps.length; i++) {
      const rep = activeReps[i];
      const repLeads = leadsData.slice(i * leadsPerRep, (i + 1) * leadsPerRep);
      ...
      <div className="space-y-4">
        <div className="text-center">
          <div className="mb-4">
            ...
            <h3 className="text-lg font-semibold text-gray-900 mb-2">Import Leads for Your Team</h3>
            <p className="text-sm text-gray-600 mb-3">
              Upload a CSV file with lead data. Leads will be evenly distributed among {activeReps.length} active sales rep{activeReps.length !== 1 ? 's' : ''}. Maximum {MAX_IMPORT_LEADS} leads per import.
            </p>
            {activeReps.length === 0 && (
              <div className="bg-red-50 border border-red-200 rounded-lg p-3 mb-3">
                <p className="text-sm text-red-800">
                  No active sales reps found. Please add sales reps before importing leads.
                </p>
              </div>
            )}
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 text-left">
              ...
              <ul className="text-xs text-blue-800 space-y-1 list-disc list-inside">
                ...
                <li>Leads will be distributed evenly among active reps</li>
              </ul>
            </div>
          </div>
          ...
          <div className="border-2 border-dashed border-gray-300 rounded-lg p-8 hover:border-red-400 transition-colors">
            <input
              ...
              className="hidden"
              id="csv-upload"
              disabled={activeReps.length === 0}
            />
            <label
              htmlFor="csv-upload"
              className={`cursor-pointer flex flex-col items-center space-y-2 ${activeReps.length === 0 ? 'opacity-50 cursor-not-allowed' : ''}`}
            >
        ...
        <div className="flex items-start space-x-3">
          ...
          <div>
            ...
            <p className="text-sm text-blue-700 mt-1">
              ...
              Found {csvData.length} rows to import and distribute evenly among your sales reps.
            </p>
          </div>
        </div>
      </div>
  ...
  const renderProcessingStep = () => (
    <div className="space-y-4 text-center py-8">
      ...
      <h3 className="text-lg font-semibold text-gray-900">Importing and Distributing Leads...</h3>
        ...
        <div className="space-y-3">
          <div className="bg-green-50 border border-green-200 rounded-lg p-4">
            <div className="flex items-center justify-between">
              <span className="text-sm font-medium text-green-900">Successfully imported and distributed</span>
  ...
  return (
    <BaseModal
      ...
      onClose={handleClose}
      title={
        step === 'upload' ? 'Import Leads for Team' :
...
export default ManagerImportLeadsModal;
